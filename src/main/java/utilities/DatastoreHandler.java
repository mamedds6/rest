...
//        studentC.addGrade( new Grade(3.0,Date.from(Instant.now()),courseC));
        final Grade gradeA = new Grade(1,3.5,Date.from(Instant.now()),courseA);
        final Grade gradeB = new Grade(2,4.5,Date.from(Instant.now()),courseB);
        final Grade gradeC = new Grade(3,2.0,Date.from(Instant.now()),courseB);
        final Grade gradeD = new Grade(4,4.0,Date.from(Instant.now()),courseC);
        final Grade gradeE = new Grade(5,5.0,Date.from(Instant.now()),courseA);
        final Grade gradeF = new Grade(6,3.0,Date.from(Instant.now()),courseC);
        studentA.addGrade(gradeA);
        studentA.addGrade(gradeB);
        studentB.addGrade(gradeC);
        studentB.addGrade(gradeD);
        studentC.addGrade(gradeE);
        studentC.addGrade(gradeF);

        datastore.save(gradeA);
        datastore.save(gradeB);
        datastore.save(gradeC);
        datastore.save(gradeD);
        datastore.save(gradeE);
        datastore.save(gradeF);
